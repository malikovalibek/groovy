...
def UsersToAdd = customfield_10032_value;

if (changedFields.contains("Соисполнители")){
    def usersFromGP = getCustomFieldValue(issue, customfield_13317)?:[];
        for (def exUser in usersFromGP){
            if (!customfield_13317_value.contains(exUser))
            UsersToAdd.addAll(customfield_13317_value)
        }
