...
def userProperty_val = userProperty.getPropertySet(currentUser).getString('jira.meta.FIO')  //
//////////////////////////////////////////////////////////////////////////////////////////////
String signer_fio = userPropertyManager.getPropertySet(currentUser).getString('jira.meta.FIO');

def UsersToAdd = [];
    
    if (userPropertyManager.getPropertySet(currentUser).getString('jira.meta.FIO') != null){
    ...
    //else{
    //     if (issueType == "Документ Внутренний"){
    //     for (def comp in arrayOfComponents){
    //         if (["Технический блок"].contains (comp.getName())) {
    //             def usersFromGP = getUsersInGroup(getGroup("Тех.блок"))
    //             for (def userGP in usersFromGP) {
